...
class DDPM(nn.Module):
    ...
    def p_sample(self, x, t, t_index):
        ...
        model_mean = sqrt_recip_alpha_t * (x - beta_t * eps_theta / sqrt_one_minus_ac_t)
        if t_index == 0:
            return model_mean
...
def run_stage(stage_name, dataset_name, epochs, pretrained_checkpoint=None):
    ...
    ddpm = DDPM(net, timesteps=train_timesteps, beta_start=beta_start, beta_end=beta_end).to(device)
