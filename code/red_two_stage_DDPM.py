...
results_root = project_dir / "results_red"
...
class RedNoiseOperator(nn.Module):
...
class DDPMRed(nn.Module):
    def __init__(self, model, timesteps=1000, beta_start=1e-4, beta_end=2e-2,
                 image_size=64, red_alpha=1.0, red_kappa0=None, red_normalize=True):
        super().__init__()
        self.model = model
        self.timesteps = timesteps
        self.noise_operator = RedNoiseOperator(
    ...
    def q_sample(self, x0, t, xi=None):
        ...
        eps_red = self.noise_operator.colorize(xi)
    ...
    def loss(self, x0):
        ...
        pred_eps_red = self.model(xt, t)
        return F.mse_loss(pred_eps_red, eps_red)
    ...
    def p_sample(self, x, t, t_index):
        ...
        eps_red_theta = self.model(x, t)
        model_mean = sqrt_recip_alpha_t * (x - beta_t * eps_red_theta / sqrt_one_minus_ac_t)

        if t_index == 0:
            return model_mean

        posterior_var_t = extract(self.posterior_variance, t, x.shape)
        z_red = self.noise_operator.colorize(torch.randn_like(x))
        return model_mean + torch.sqrt(posterior_var_t) * z_red
    ...
    def sample_ddim(self, shape, device, sample_steps=100):
        ...
        for i, t_now in enumerate(schedule):
            ...
            x0_pred = (x - torch.sqrt(1.0 - alpha_bar_t) * eps_red) / torch.sqrt(alpha_bar_t)
            x0_pred = x0_pred.clamp(-1, 1)
            x = torch.sqrt(alpha_bar_prev) * x0_pred + torch.sqrt(1.0 - alpha_bar_prev) * eps_red
...
def make_run_config(dataset_info):
    return {
        ...
        "beta_end": beta_end,
        "red_alpha": red_alpha,
        "red_kappa0": red_kappa0,
        "red_normalize": red_normalize,
